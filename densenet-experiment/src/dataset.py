...
class OralCancerDataset(Dataset):
    ...
    def __getitem__(self, idx):
        ...
        # Load image
        # Convert to RGBA first to handle Palette images with transparency, then to RGB
        ...
        if label == 0 and self.extra_normal_aug:  # Normal class
            # Define extra aggressive augmentation for Normal (REMOVED NOISE)
            extra_aug = A.Compose([
                A.RandomBrightnessContrast(brightness_limit=0.4, contrast_limit=0.4, p=0.8),
                A.HueSaturationValue(hue_shift_limit=25, sat_shift_limit=40, val_shift_limit=25, p=0.7),
                # Removed GaussianBlur and GaussNoise to prevent artifacts
...
def get_transforms(train=True, img_size=224):
    if train:
        return A.Compose([
            ...
            # var_limit=(10.0, 50.0) corresponds to std approx 3.2-7.1.
            # Normalized by 255: 0.012-0.028
            A.GaussNoise(std_range=(0.012, 0.028), p=0.4),
