...
# ============================================
# DENSENET + RESNET HYBRID: DenseNet121 + ResNet50 Fusion
# Description: Concatenation fusion of DenseNet121 (dense local connectivity)
#              and ResNet50 (deep residual features).
# Architecture: DenseNet121 (1024) + ResNet50 (2048) → Concat (3072) → Classifier
...
EXPERIMENT_NAME = 'densenet_resnet'
...
class DenseNetResNetHybrid(nn.Module):
    """
    Hybrid: DenseNet121 (1024) + ResNet50 (2048) → Concat (3072) → Classifier
    DenseNet captures fine-grained texture, ResNet captures spatial hierarchy.
    """

    def __init__(self, num_classes=2, pretrained=True):
        super(DenseNetResNetHybrid, self).__init__()
        ...
        # ---- Fusion ----
        self.fusion_dim = densenet_dim + resnet_dim  # 1024 + 2048 = 3072
    ...
    def forward(self, x):
        ...
        # Concat fusion
        combined = torch.cat((dn_feat, resnet_feat), dim=1)  # (B, 3072)
...
def main():
    ...
    print(f"   Description: DenseNet121 + ResNet50 Hybrid (Optimized)")
    ...
    # Model
    model = DenseNetResNetHybrid(num_classes=NUM_CLASSES).to(device)
    ...
    backbone_params = list(model.densenet_features.parameters()) + list(model.resnet_features.parameters())
