...
from torchvision.models import DenseNet121_Weights, Swin_V2_B_Weights
import numpy as np
...
# ============================================
# DENSENET + SWIN HYBRID: DenseNet121 + SwinV2-Base
# Description: Concatenation fusion of DenseNet121 local features
#              and SwinV2-Base global features for oral cancer detection.
# Architecture: DenseNet121 (1024) + SwinV2-B (1024) → Concat (2048) → Classifier
...
EXPERIMENT_NAME = 'densenet_swintransformer'
...
class DenseNetSwinTransformerHybrid(nn.Module):
    """Hybrid model fusing DenseNet121 (local) and SwinV2-Base (global) features via concatenation."""

    def __init__(self, num_classes=2, pretrained=True):
        super(DenseNetSwinTransformerHybrid, self).__init__()

        # 1. DenseNet121 backbone (local features)
        if pretrained:
        ...
        # 2. SwinV2-Base backbone (global features)
        if pretrained:
            self.swin = models.swin_v2_b(weights=Swin_V2_B_Weights.DEFAULT)
        else:
            self.swin = models.swin_v2_b(weights=None)
            
        self.swin.head = nn.Identity()
        swin_dim = 1024

        # 3. Fusion: Concatenation
        self.fusion_dim = densenet_dim + swin_dim  # 2048
    ...
    def forward(self, x):
        ...
        combined = torch.cat((dn_feat, swin_feat), dim=1)  # (B, 2048)
...
def main():
    ...
    print(f"   Description: DenseNet121 + SwinV2-Base Hybrid (Optimized)")
    ...
    # Model
    model = DenseNetSwinTransformerHybrid(num_classes=NUM_CLASSES).to(device)
    ...
    backbone_params = list(model.densenet_features.parameters()) + list(model.swin.parameters())
