...
# ============================================
# AXIAL + DENSENET HYBRID: Axial Attention + DenseNet121 Fusion
# Description: Concatenation fusion of DenseNet121 features (dense local)
#              and Axial Attention features (global row/column context).
# Architecture: DenseNet121 (1024) + Axial Branch (256) → Concat (1280) → Classifier
...
EXPERIMENT_NAME = 'axial_densenet'
...
class AxialDenseNetHybrid(nn.Module):
    """
    Hybrid: DenseNet121 (1024) + Axial Attention (256) → Concat (1280) → Classifier
    """

    def __init__(self, num_classes=2, pretrained=True, num_axial_blocks=4, axial_channels=256, num_heads=8):
        super(AxialDenseNetHybrid, self).__init__()
        ...
        # ---- Branch 2: Axial Attention (ResNet stem) ----
        if pretrained:
        ...
        # ---- Fusion ----
        self.fusion_dim = densenet_dim + axial_dim  # 1024 + 256 = 1280
    ...
    def forward(self, x):
        ...
        # Concat fusion
        combined = torch.cat((dn_feat, axial_feat), dim=1)  # (B, 1280)
...
def main():
    ...
    print(f"   Description: Axial Attention + DenseNet121 Hybrid (Optimized)")
    ...
    # Model
    model = AxialDenseNetHybrid(num_classes=NUM_CLASSES).to(device)
    ...
    backbone_params = list(model.densenet_features.parameters()) + list(model.axial_stem.parameters())
