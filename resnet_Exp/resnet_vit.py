...
# ============================================
# RESNET + ViT HYBRID: ResNet50 + ViT-Base Fusion
# Description: Concatenation fusion of ResNet50 local features
#              and ViT-Base global features for oral cancer detection.
# Architecture: ResNet50 (2048) + ViT-Base (768) → Concat (2816) → Classifier
...
class ResNetViTHybrid(nn.Module):
    """Hybrid model fusing ResNet50 (local) and ViT-Base (global) features via concatenation."""
    ...
    def __init__(self, num_classes=2, pretrained=True):
        ...
        # 2. ViT-Base backbone (global features) — img_size=256 via timm
        ...
        self.fusion_dim = resnet_dim + vit_dim  # 2816
    ...
    def forward(self, x):
        ...
        combined = torch.cat((resnet_feat, vit_feat), dim=1)  # (B, 2816)
...
def main():
    ...
    print(f"   Description: ResNet50 + ViT-Base Hybrid (Optimized)")
    ...
    backbone_params = list(model.resnet_features.parameters()) + list(model.vit.parameters())
