...
from torchvision.models import ResNet50_Weights, Swin_V2_B_Weights
import numpy as np
...
# ============================================
# RESNET + SWIN HYBRID: ResNet50 + SwinV2-Base
# Description: Concatenation fusion of ResNet50 local features
#              and SwinV2-Base global features for oral cancer detection.
# Architecture: ResNet50 (2048) + SwinV2-B (1024) → Concat (3072) → Classifier
...
EXPERIMENT_NAME = 'resnet_swintransformer'
...
class ResNetSwinTransformerHybrid(nn.Module):
    """Hybrid model fusing ResNet50 (local) and SwinV2-Base (global) features via concatenation."""

    def __init__(self, num_classes=2, pretrained=True):
        super(ResNetSwinTransformerHybrid, self).__init__()

        # 1. ResNet50 backbone (local features)
        if pretrained:
        ...
        self.fusion_dim = resnet_dim + swin_dim  # 3072
    ...
    def forward(self, x):
        ...
        combined = torch.cat((resnet_feat, swin_feat), dim=1)  # (B, 3072)
...
def main():
    ...
    print(f"   Description: ResNet50 + SwinV2-Base Hybrid (Optimized)")
    ...
    # Model
    model = ResNetSwinTransformerHybrid(num_classes=NUM_CLASSES).to(device)
    ...
    backbone_params = list(model.resnet_features.parameters()) + list(model.swin.parameters())
