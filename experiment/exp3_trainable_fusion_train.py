...
# ============================================
# EXPERIMENT 3: Trainable (Gated) Fusion
# Description: Uses a learnable gating mechanism to weight features before classification.
# Comparison: Tests if the model can learn *which* features (local vs global) are more important per image.
# Architectures: DenseNet121 + ViT-Base
# Fusion: Gated Fusion (Sigmoid activation on concatenated correlation)
# ============================================
...
EXPERIMENT_NAME = 'exp3_trainable_fusion'
...
class HybridViTDenseNet(nn.Module):
    def __init__(self, num_classes=2, pretrained=True):
        ...
        # 3. Fusion: Trainable Gating
        self.fusion_dim = densenet_dim + vit_dim
        
        # Gating network: Takes concatenated features -> outputs weights [0, 1] for each feature
        self.gate_layer = nn.Sequential(
            nn.Linear(self.fusion_dim, self.fusion_dim),
            nn.Sigmoid()
        )
        
        print(f"  Hybrid: Gated Fusion (Learnable weights for {self.fusion_dim} features)")
    ...
    def forward(self, x):
        ...
        # Calculate Gates
        gates = self.gate_layer(combined) # (B, 1792)
        
        # Apply Gates (Element-wise multiplication)
        gated_features = combined * gates
        
        return self.classifier(gated_features)

# Training Utils (Duplicated for standalone capability)
