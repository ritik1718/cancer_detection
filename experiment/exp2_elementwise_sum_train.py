...
# ============================================
# EXPERIMENT 2: Element-wise Sum Fusion
# Description: Project DenseNet features to match ViT dims and sum them element-wise.
# Comparison: Tests if simple additive fusion is better than concatenation.
# Architectures: DenseNet121 (Local) + ViT-Base (Global)
# Fusion: Element-wise Sum (768 dims)
# ============================================
...
EXPERIMENT_NAME = 'exp2_elementwise_sum'
...
class HybridViTDenseNet(nn.Module):
    def __init__(self, num_classes=2, pretrained=True):
        ...
        # --- Fusion Specifics ---
        # Project DenseNet dim (1024) to ViT dim (768)
        self.densenet_proj = nn.Linear(densenet_dim, vit_dim)
        
        # Output info
        print(f"  Hybrid: Element-wise Sum Fusion (Dim: {vit_dim})")
    ...
    def forward(self, x):
        ...
        # Project DenseNet
        dn_proj = self.densenet_proj(dn_feat) # (B, 768)
        
        # ViT
        vit_feat = self.vit(x) # (B, 768)
        
        # Element-wise Sum
        combined = dn_proj + vit_feat
        
        return self.classifier(combined)


# --- Training Utils (Same as exp1) ---
