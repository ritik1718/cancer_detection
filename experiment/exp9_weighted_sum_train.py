...
from src.datasets.oral_cancer_dataset import OralCancerDataset, get_transforms # Safety
from src.dataset import OralCancerDataset, get_transforms

# ============================================
# EXPERIMENT 9: Weighted Sum Fusion
# Description: Learnable weighted sum of features.
# Comparison: Allows model to learn a global preference between local (DenseNet) and global (ViT) features.
# Architectures: DenseNet121 + ViT-Base
# Fusion: Weighted Sum: alpha * DenseNet + (1-alpha) * ViT
# ============================================
...
EXPERIMENT_NAME = 'exp9_weighted_sum'
...
class HybridViTDenseNet(nn.Module):
    def __init__(self, num_classes=2, pretrained=True):
        ...
        # Proj DenseNet to match ViT
        self.densenet_proj = nn.Linear(densenet_dim, vit_dim)
        
        # Learnable Weight Parameter (initialized to 0.5)
        self.alpha = nn.Parameter(torch.tensor(0.5))
        
        # Fusion Output Dim is same as ViT dim
        self.fusion_dim = vit_dim
        
        print(f"  Hybrid: Weighted Sum Fusion (Init Alpha: 0.5)")
    ...
    def forward(self, x):
        ...
        # Project
        dn_proj = self.densenet_proj(dn_feat)
        
        vit_feat = self.vit(x)
        
        # Weighted Sum (sigmoid on alpha to keep ratio in [0,1] or simple scalar)
        # Using sigmoid to ensure it stays between 0 and 1
        weight = torch.sigmoid(self.alpha)
        combined = weight * dn_proj + (1 - weight) * vit_feat
...
def main():
    ...
    for epoch in range(NUM_EPOCHS):
        ...
        print(f"  Current Mixing Weight (Alpha): {alpha_val:.4f} (DenseNet) / {1-alpha_val:.4f} (ViT)")
