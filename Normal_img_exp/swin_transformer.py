...
# ============================================
# SWIN TRANSFORMER SOLO: SwinV2-Base
# Description: Solo Swin Transformer V2 Base for oral cancer detection.
# Architecture: SwinV2-Base (1024) → Classifier
...
EXPERIMENT_NAME = 'swin_transformer'
...
class SwinTransformerSolo(nn.Module):
    """Solo Swin Transformer V2 Base model with custom classification head."""

    def __init__(self, num_classes=2, pretrained=True):
        super(SwinTransformerSolo, self).__init__()

        # 1. SwinV2 backbone (global features)
        if pretrained:
            self.swin = models.swin_v2_b(weights=Swin_V2_B_Weights.DEFAULT)
        else:
            self.swin = models.swin_v2_b(weights=None)
            
        # Remove original head to output raw Features (1024 dims)
        self.swin.head = nn.Identity()
        swin_dim = 1024

        # 2. Enhanced classifier head
...
def main():
    ...
    print(f"   Description: Swin Transformer V2 Base Solo (Optimized)")
    ...
    # Model
    model = SwinTransformerSolo(num_classes=NUM_CLASSES).to(device)
