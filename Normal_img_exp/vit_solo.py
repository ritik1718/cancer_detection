...
# ============================================
# VIT SOLO: Standalone ViT-Base
# Description: Fine-tuned ViT-Base (ImageNet pretrained) for oral cancer detection.
# Architecture: ViT-B/16 → CLS Token → Enhanced Classifier
# Feature dim: 768
...
class ViTSolo(nn.Module):
    """Standalone ViT-Base fine-tuned for binary oral cancer classification."""

    def __init__(self, num_classes=2, pretrained=True):
        super(ViTSolo, self).__init__()

        # ViT-Base backbone via timm (handles variable image sizes easily)
...
def main():
    ...
    print(f"   Description: Standalone ViT-Base Fine-tuned")
    ...
    # Model
    model = ViTSolo(num_classes=NUM_CLASSES).to(device)
