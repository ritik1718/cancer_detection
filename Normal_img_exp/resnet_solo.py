...
# ============================================
# RESNET SOLO: Standalone ResNet50
# Description: Fine-tuned ResNet50 (ImageNet pretrained) for oral cancer detection.
# Architecture: ResNet50 → Global Avg Pool → Enhanced Classifier
# Feature dim: 2048
...
class ResNetSolo(nn.Module):
    """Standalone ResNet50 fine-tuned for binary oral cancer classification."""

    def __init__(self, num_classes=2, pretrained=True):
        super(ResNetSolo, self).__init__()
        ...
        self.features = nn.Sequential(*list(self.resnet.children())[:-1])  # Output: (B, 2048, 1, 1)
        resnet_dim = 2048
...
def main():
    ...
    print(f"   Description: Standalone ResNet50 Fine-tuned (Optimized)")
    ...
    # Model
    model = ResNetSolo(num_classes=NUM_CLASSES).to(device)
