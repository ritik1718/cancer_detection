...
# ============================================
# MOBILENET + ViT HYBRID: MobileNetV2 + ViT-Base Fusion
# Description: Concatenation fusion of MobileNetV2 features (efficient local)
#              and ViT-Base features (global attention) for oral cancer detection.
# Architecture: MobileNetV2 (1280) + ViT-Base (768) → Concat (2048) → Enhanced Classifier
...
class MobileNetViTHybrid(nn.Module):
    """
    Hybrid model fusing MobileNetV2 (efficient local features)
    with ViT-Base (global attention features) via concatenation.
    """

    def __init__(self, num_classes=2, pretrained=True):
        super(MobileNetViTHybrid, self).__init__()

        # ---- Branch 1: MobileNetV2 (efficient local features) ----
        if pretrained:
            mobilenet = models.mobilenet_v2(weights=MobileNet_V2_Weights.IMAGENET1K_V1)
        else:
            mobilenet = models.mobilenet_v2(weights=None)

        self.mobilenet_features = mobilenet.features   # Output: (B, 1280, H/32, W/32)
        self.mobilenet_pool = nn.AdaptiveAvgPool2d((1, 1))
        mobilenet_dim = 1280

        # ---- Branch 2: ViT-Base (global attention) ----
        self.vit = timm.create_model('vit_base_patch16_224', pretrained=pretrained, num_classes=0, img_size=IMG_SIZE)
        vit_dim = 768

        # ---- Fusion: Concatenation ----
        self.fusion_dim = mobilenet_dim + vit_dim  # 1280 + 768 = 2048
    ...
    def forward(self, x):
        ...
        mob_feat = self.mobilenet_pool(mob_feat)
        mob_feat = torch.flatten(mob_feat, 1)

        # Branch 2: ViT-Base
        vit_feat = self.vit(x)

        # Fusion
        combined = torch.cat((mob_feat, vit_feat), dim=1)
...
def main():
    ...
    print(f"   Description: MobileNetV2 + ViT-Base Hybrid (Optimized)")
    print(f"   Architecture: MobileNetV2 (1280) + ViT-Base (768) → Concat (2048) → Classifier")
    ...
    # Model
    model = MobileNetViTHybrid(num_classes=NUM_CLASSES).to(device)
    ...
    backbone_params = list(model.mobilenet_features.parameters()) + list(model.vit.parameters())
