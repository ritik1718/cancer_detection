...
# ============================================
# DENSENET SOLO: Standalone DenseNet121
# Description: Fine-tuned DenseNet121 (ImageNet pretrained) for oral cancer detection.
# Architecture: DenseNet121 → Global Avg Pool → Enhanced Classifier
# Feature dim: 1024
...
class DenseNetSolo(nn.Module):
    """Standalone DenseNet121 fine-tuned for binary oral cancer classification."""

    def __init__(self, num_classes=2, pretrained=True):
        super(DenseNetSolo, self).__init__()
        ...
        # 1024 is the feature dimension for densenet121
        densenet_dim = 1024

        # Enhanced classifier head
        self.classifier = nn.Sequential(
            nn.Linear(densenet_dim, 512),
            nn.BatchNorm1d(512),
            nn.ReLU(),
            nn.Dropout(0.5),
            nn.Linear(512, 256),
...
def main():
    ...
    print(f"   Description: Standalone DenseNet121 Fine-tuned")
    ...
    # Model
    model = DenseNetSolo(num_classes=NUM_CLASSES).to(device)
